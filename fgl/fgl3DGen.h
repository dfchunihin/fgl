...
public:
	...
	long width;
	long height;
	fglRECT(const long& pnLeft=0, const long& pnTop=0, const long& pnWidth=0, const long& pnHeight=0) {left=pnLeft; top=pnTop; width=pnWidth; height=pnHeight;}
...
public:
	float a1,a2,a3,a4;
	fglROW4(const float& pa1=0.0f, const float& pa2=0.0f, const float& pa3=0.0f, const float& pa4=0.0f) {a1=pa1;a2=pa2;a3=pa3;a4=pa4;}
...
public:
	...
	fglQUAT(void) {build(1.0f,0.0f,0.0f,0.0f);}
...
public:
	...
	fglCOLOR(void) { r=g=b=0.0f; a=0.0f; }
...
public:
	...
	fglVECTOR pos;
	fglVECTOR scale;
	fglTRANSFORMConstraints(void): rot(1.0f, 1.0f, 1.0f), pos(1.0f,1.0f, 1.0f), scale(1.0f,1.0f,1.0f) {}
};
...
public:
	...
	fglTRANSFORM(void) { loadIdentity(); }
...
public:
	...
	fglVECTOR getSize(void) const { return max - min; }
	fglVECTOR getCenter(void) const { return (min+max) * 0.5f; }
...
{
private:
	fglBBox(const fglBBox& poSrc) : tr(poSrc.tr), trg(poSrc.trg) {}
...
public:
	...
	fglVECTOR max;
	bool isVisible;
	unsigned meshType; //0 - no mesh, 1 - cube, 2 - sphere
	fglCOLOR meshColor;
	bool meshFill;
	float meshLineWidth;
	unsigned meshLighting; //lighting mode: 0 - none, 1 - phong, 2 -gouraud

	fglBBox(const fglTRANSFORM& poTR, const fglTRANSFORM& poTRG);
	const fglBBox& operator=(const fglBBox& poSrc);
	virtual ~fglBBox(void);
	...
	static enum planes {TOP = 0, BOTTOM, LEFT, RIGHT, NEARP, FARP};
