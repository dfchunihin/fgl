...
{
	if (poParent) if ( poParent->exist() && dynamic_cast<fglJoint*>(poParent->map())==NULL ) return setParentValue(poParent); 
...
{
	if (poParent) if ( poParent->exist() && dynamic_cast<fglJoint*>(poParent->map())==NULL ) return false;
