...
void fglFontManager::setScene(const fglREF<fglScene>& poScene)
{ 
...
void fglFont::setScene(const fglREF<fglScene>& poScene)
{ 
	if ( oScene.empty() ) oScene = poScene;
}
...
const fglFontGlyph* fglFont::getGlyph(fglWChar pChar)
{
	...
	if (liGlyph != aGlyphs.end()) return &liGlyph->second;
	
	fglFontGlyph& loGlyph = aGlyphs[pChar];
...
fglFontGlyph::fglFontGlyph(void)
{
	width = 0.0f;
	height = 0.0f;
	advanceHor = 0.0f;
...
void fglFontGlyph::load(const fglWChar& pChar, const FT_Face poFace)
{
	...
	short lnContours = loOutline.n_contours;
