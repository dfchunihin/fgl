...
void fglModel::copySkel(const fglModel* poModel, fglBITFLAG pnShare)
{
	...
	const fglJoint* loRootJoint = duplicate<fglJoint>( poModel->oRootJoint.map(), (pnShare.check(fglSHARE_SKEL) ? fglDUPLICATE_INSTANCE : fglDUPLICATE_COPY), pnShare );
	if ( ! loRootJoint ) return;
	setRootJoint( loRootJoint->getRef() );
}
...
void fglModel::copyAnim(const fglModel* poModel, fglBITFLAG pnShare)
{
	...
	aAnims.copy(&poModel->aAnims, pnShare.check(fglSHARE_ANIM));
}
...
void fglModel::buildLOD(float pnViewRange, float pnStep, unsigned pnSteps) 
{
	...
	static fglCollection<fglModel> laLODs;
	...
	public:
		...
		static void onMesh(fglWalkNodes* poHandler, fglNode* poNode, fglVisual* poVisual)
		{
			...
			while (lnLODs--)
			{
				...
				for (i=0; i!=lnBones; i++)
				{
					loBone = &loMesh->aBones[i];
					loBone->setParent(NULL);
