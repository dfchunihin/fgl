...
void fglCollider::call(fglEvent& poEvent)
{
	...
	if (poEvent.name == fglEVENT_testNode) 
	{
		...
		const fglREF<fglCollideInfo>* loInfoRef = getBind<fglCollideInfo>(_GetValue2i(poEvent.aParams[1]));
		fglCollideInfo* loInfo = loInfoRef ? loInfoRef->map() : NULL;
		if (!loNode) return;
		if (!loNode->map()) return;
		_RetValue2l( testNode(loNode->get(), loInfo) ); 
		return;
	}
...
bool fglCollider::testNode(const fglNode& poNode, fglCollideInfo* poInfo) const
{
	...
	{
		if ( (loTestNode = aNodes[i].map()) == NULL ) continue;
...
void fglCollideInfo::call(fglEvent& poEvent)
{
	...
	if (poEvent.name == fglEVENT_hNode1) { if (oNode1.empty()) _RetValue2i(0); else _RetValue2i(oNode1->getHFGL()); return; }
	if (poEvent.name == fglEVENT_hNode2) { if (oNode2.empty()) _RetValue2i(0); else _RetValue2i(oNode2->getHFGL()); return; }
	...
	{ 
		...
		if (poEvent.name == fglEVENT_getNode1) { if (loNode = loPair->oNode1.map()) _RetValue2i( loNode->getHFGL() ); return; }
		if (poEvent.name == fglEVENT_getNode2) { if (loNode = loPair->oNode2.map()) _RetValue2i( loNode->getHFGL() ); return; }
