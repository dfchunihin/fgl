...
void fglNode::call(fglEvent& poEvent)
{
	...
	{
		static fglArray<fglVisual*> laVisuals;
		laVisuals.reserve(100);
		laVisuals.clear();

		static fglString lcVisuals;
		lcVisuals.clear();
	...
	{
		static fglArray<fglMaterial*> laMaterials;
		laMaterials.reserve(100);
		laMaterials.clear();

		static fglString lcMaterials;
		lcMaterials.clear();
...
bool fglNode::copy(const fglBind* poSrc, fglBITFLAG pnShare)
{
	...
	const fglNode* loNewChild;
	for (unsigned i=0; i!=loNode->aChilds.size(); i++)
	{
		loChild= loNode->aChilds[i];
		if ( loChild.empty() ) continue;
		
		if ( loNewChild = duplicate<fglNode>(loChild.map(), fglDUPLICATE_COPY, pnShare) )
		{
			aChilds.add( loNewChild->getRef() );
		}
	...
	const fglVisual* loNewVisual;
	for (unsigned i=0; i!=loNode->aVisuals.size(); i++)
	{
		loVisual = loNode->aVisuals[i];
		...
		else if ( loNewVisual = duplicate<fglVisual>(loVisual.map(), (pnShare.check(fglSHARE_AVISUALS) ? fglDUPLICATE_INSTANCE : fglDUPLICATE_COPY), pnShare) )
		{
			aVisuals.add(loNewVisual->getRef());
			gaCopyVisuals[loVisual->getHFGL()] = loNewVisual->getRef();
		}
	}
...
{
	fglREF<fglNode> loParent;
	if (poParent) loParent = *poParent;
	if (oParent.map() == loParent.map()) return true;
	fglNode* loNode = dynamic_cast<fglNode*>(loParent.map());
	if ( loParent.exist() && loNode==NULL ) return false;
	while (loNode) { if (this == loNode) return false; loNode = loNode->getParent().map(); }
	if (oParent.exist()) oParent->aChilds.del(getRef());
	oParent = loParent;
	buildTransform();
	return true;
}
...
bool fglNode::setParent(const fglREF<fglNode>* poParent)
{
	...
	if (oParent.exist()) oParent->aChilds.add(getRef());
	return true;
}
...
void fglNode::buildTransform(void)
{
	...
	if (lnChans!=0)
	{
		for (unsigned i=0; i!=lnChans; i++) 
			if (aAnimChans[i].exist()) trg = aAnimChans[i]->tr;
	}
	if (oParent.exist())  trg = (oParent->trg * trgConstraints) * trg; 
	...
	while (lnChilds--) { loChild = aChilds[lnChilds].map(); if (loChild) loChild->buildTransform(); }
...
void fglNode::calcBounds(void)
{
	...
	while (lnVisuals--)
	{
		loVisual = aVisuals[lnVisuals].map();
